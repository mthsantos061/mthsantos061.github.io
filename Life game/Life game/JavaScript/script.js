    ...
    for (let i = 0; i < window.setupPlayerCount; i++) {
        container.innerHTML += `
            <div style="margin-bottom:10px;">
                ...
                <label>Cor: 
                    <input type="color" id="player-color-${i}" value="${playerColors[i % playerColors.length]}">
                </label>
            </div>
        `;
    }
            ...
            if (idx >= 0 && idx < board.length) {
                ...
                if (space.type === "property") {
                    ...
                    if (space.owner !== undefined) {
                        ...
                        // Pinta a célula com a cor do jogador dono
                        cell.style.background = players[space.owner].color;
                        // Borda mais grossa e destacada
                        cell.style.border = `4px solid ${players[space.owner].color}`;
                        // Deixa o texto branco para contraste
                        cell.style.color = '#fff';
                        // Sombra para destacar
                        cell.style.boxShadow = `0 0 12px 2px ${players[space.owner].color}55`;
                    }
                }
